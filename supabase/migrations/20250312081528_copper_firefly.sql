...
-- Add missing RLS policies for blog_posts
...
ALTER TABLE blog_posts ADD CONSTRAINT check_excerpt_length CHECK (length(excerpt) <= 500);
ALTER TABLE comments ADD CONSTRAINT check_content_not_empty CHECK (length(content) > 0);
...
-- Add email validation to profiles
ALTER TABLE profiles
ADD CONSTRAINT check_email_format
CHECK (email IS NULL OR public.is_valid_email(email));

-- Add function to validate phone format
...
-- Add phone validation to profiles
ALTER TABLE profiles
ADD CONSTRAINT check_phone_format
CHECK (phone IS NULL OR public.is_valid_phone(phone));

-- Add function to validate IBAN format
...
-- Add IBAN validation to profiles
ALTER TABLE profiles
